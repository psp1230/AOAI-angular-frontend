...
export class ChatComponent implements OnInit {
  ...
  configuration = {
    ...
    azureEndpoint: '',
    deployment: '',
    deployments: [{ name: 'gpt-4o', description: 'OpenAI gpt-4o', threshold: '10000' }],
  ...
  getExamplePrompts(): any[] {
    ...
    examples.push({
      prompt: '如何在 Python 中重複執行一系列項目？',
      icon: this.iconLegal,
      colour: this.getRandomColor()
    } as any);

    examples.push({
      prompt: '幫我更新我的網站追蹤程式碼。',
  ...
  async sendDocuments() {
    ...
    try {
      ...
      this.messages += `<div class="alert alert-info">較大的 PDF、Word、純文字或 Markdown 文件可能需要較長的處理時間。<span class="mt-4 timestamp timestamp-system">${this.getTimestamp()}</span></div>`;
